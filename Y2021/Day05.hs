module AdventOfCode.Y2021.Day5 where

import AdventOfCode.Common.Parser
import AdventOfCode.Common.List (freq, count)
import AdventOfCode.Common.Grid

--------------------------------------------------------------------------------

type Value = Int
type Coords = (Value, Value)
data LineSegment = LineSegment
   { start :: Coords
   , end :: Coords
   } deriving (Eq, Show)

newtype Intersections = Intersections Int
instance Show Intersections where
   show (Intersections 0) = "."
   show (Intersections n) = show n

x1 (LineSegment (x,_) _) = x
y1 (LineSegment (_,y) _) = y
x2 (LineSegment _ (x,_)) = x
y2 (LineSegment _ (_,y)) = y

horizontal seg = y1 seg == y2 seg
vertical seg = x1 seg == x2 seg
orthogonal seg = horizontal seg || vertical seg
point seg = horizontal seg && vertical seg
...
expand seg = let
   ...
   dirY = signum (y2 seg - y1 seg)
   start' = (x1 seg + dirX, y1 seg + dirY)
   in start seg : expand seg {start = start'}
