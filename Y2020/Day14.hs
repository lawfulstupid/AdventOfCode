...
module AdventOfCode.Y2020.Day14 where

import AdventOfCode.Y2020.Common
import AdventOfCode.Y2020.Word36
...
data Instruction = Write Word36 Word36 | SetMask Bitmask
   deriving (Show)
data Memory = Mem Bitmask (Map Word36 Word36)
   deriving (Show)
type Bitmask = String
...
parse :: Input -> Program
parse = map $ parseUsing parseInstuction

parseInstuction :: Parser Instruction
parseInstuction = parseInstructionWrite <|> parseInstructionSetMask

parseInstructionWrite :: Parser Instruction
...
parseInstructionSetMask :: Parser Instruction
parseInstructionSetMask = do
   "mask = " <- string
   bitmask <- parseBitmask
   return $ SetMask bitmask

parseBitmask :: Parser Bitmask
parseBitmask = do
   mask <- string
   guard (length mask == 36)
   return mask


applyMask1 :: Bitmask -> Word36 -> Word36
applyMask1 mask x = let
   cnj = readBin $ flip map mask $ \case {'X' -> '1'; x -> x}
   dsj = readBin $ flip map mask $ \case {'X' -> '0'; x -> x}
   in cnj .&. (x .|. dsj)

write :: Word36 -> Word36 -> Memory -> Memory
write adr dat (Mem mask mem) = Mem mask $ M.insert adr dat mem

run1 :: Program -> Memory
run1 = aux (Mem emptyMask M.empty) where
   aux :: Memory -> Program -> Memory
   aux mem [] = mem
   aux (Mem mask mem) (i:p) = case i of
      SetMask mask' -> aux (Mem mask' mem) p
      Write adr dat -> aux (write adr (applyMask1 mask dat) $ Mem mask mem) p

part1 :: Input -> Integer
part1 input = let
   Mem _ m = run1 $ parse input
...
applyMask2 mask x = let
   ...
   in map readBin $ combine xstr mask
   where
   combine :: String -> String -> [String]
   combine [] [] = [[]]
   combine (b:bs) (m:ms) = let c = combine bs ms in case m of
      '0' -> map (b:) c
      '1' -> map ('1':) c
      'X' -> map ('0':) c ++ map ('1':) c

writeAll :: [Word36] -> Word36 -> Memory -> Memory
writeAll [] _ m = m
writeAll (adr:adrs) dat m = writeAll adrs dat $ write adr dat m

run2 :: Program -> Memory
run2 = aux (Mem emptyMask M.empty) where
   aux :: Memory -> Program -> Memory
   aux m [] = m
   aux (Mem mask mem) (i:p) = case i of
      SetMask mask' -> aux (Mem mask' mem) p
      Write adr dat -> aux (writeAll (applyMask2 mask adr) dat $ Mem mask mem) p

part2 :: Input -> Integer
part2 input = let
   Mem _ m = run2 $ parse input
   in sum $ map (toInteger . snd) $ M.assocs m
