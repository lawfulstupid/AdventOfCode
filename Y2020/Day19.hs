...
module AdventOfCode.Y2020.Day19 where
...
parseFromRule :: Rule -> Parser Message
parseFromRule (Lit s) = do
   m <- string
   guard (s == m)
   return m
parseFromRule (Seq []) = pure ""
parseFromRule (Seq (r:rs)) = do
   x <- parseFromRule r
   xs <- parseFromRule (Seq rs)
   return (x ++ xs)
parseFromRule (Alt r1 r2) = parseFromRule r1 <|> parseFromRule r2

getRule :: RuleSet -> Int -> Rule
getRule rs n = let
   raw = M.findWithDefault undefined n rs
   in reduce raw
   where
   reduce :: Rule -> Rule
   reduce (Lit s) = Lit s
   reduce (Ref xs) = Seq $ map (getRule rs) xs
   reduce (Alt r1 r2) = Alt (reduce r1) (reduce r2)

simplify :: Rule -> Rule
simplify (Lit s) = Lit s
simplify (Alt r1 r2) = Alt (simplify r1) (simplify r2)
simplify (Seq s) = case simplifySeq s of {[] -> Lit ""; [x] -> x; xs -> Seq xs}
   where
   simplifySeq :: [Rule] -> [Rule]
   simplifySeq [] = []
   simplifySeq [r] = [simplify r]
   simplifySeq (x:y:xs) = case (simplify x, simplify y) of
      (Lit a, Lit b) -> simplifySeq ((Lit (a++b)) : xs)
      (a, Lit b) -> a : simplifySeq (Lit b : xs)
      (a, b) -> a : b : simplifySeq xs

parseInput :: [String] -> Input
parseInput input = let
   [rulesStr,messages] = breakAll null input
   rules = map (parseUsing parseRuleLine) rulesStr
   in (M.fromList rules, messages)

parseRuleLine :: Parser RuleLine
parseRuleLine = do
   idx <- reader
   ':' <- char
   ' ' <- char
   ...
   parseRule = parseRule' <|> parseRuleAlt
   
   parseRule' :: Parser Rule
   parseRule' = parseRuleLit <|> parseRuleRef
   
   parseRuleLit :: Parser Rule
   parseRuleLit = do
      '"' <- char
      s <- string
      '"' <- char
...
part1 :: Input -> Int
part1 (rules, messages) = let
   !rule = simplify $ getRule rules 0
   parser = parseFromRule rule
   total = length messages
   in count isJust $ do
      (idx,message) <- zip [1..] messages
      let !_ = debug True $ putStrLn ("Parsing " ++ show idx ++ "/" ++ show total ++ ": " ++ message)
      return $ parseMaybe parser message

-- part2 :: Input -> Int
part2 (rules, messages) = let
   !rules' = M.insert 8 (Alt (Ref [42]) (Ref [42,8])) 
      $ M.insert 11 (Alt (Ref [42,31]) (Ref [42,11,31]))
      $ rules
   in part1 (rules', messages)

type Input = (RuleSet, [Message])
