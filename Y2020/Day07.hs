...
module AdventOfCode.Y2020.Day7 where
...
data Rule = Rule {container :: Description, contents :: Contents}
   deriving (Show, Eq)

parse :: String -> Rule
parse = parseUsing parseRule

parseRule :: Parser Rule
parseRule = do
   container <- parseDescription
   " bags contain " <- string
   list <- parseContents
   '.' <- char
   return $ Rule container list

parseContents :: Parser Contents
parseContents = parseEmptyContents <|> do
   quantity <- reader
   ' ' <- char
   ...
   list <- pure [] <|> do
      ", " <- string
      parseContents
   return ((quantity,desc) : list)

parseEmptyContents :: Parser Contents
parseEmptyContents = do
   "no other bags" <- string
   return []
...
canContain :: Description -> Rule -> Bool
canContain myBag rule = any (\(_,bag) -> bag == myBag) $ contents rule

findRulesFor :: Description -> [Rule] -> [Rule]
findRulesFor myBag rules = filter (\rule -> container rule == myBag) rules

potentialContainers :: [Rule] -> Description -> [Description]
potentialContainers [] _ = []
potentialContainers rules myBag = let
   (immediate,maybe) = partition (canContain myBag) rules
   nonImmediate = immediate >>= potentialContainers maybe . container
   in nub (map container immediate ++ nonImmediate)
   
part1 :: Input -> Int
part1 input = length $ potentialContainers (map parse input) "shiny gold"


buildMap :: [Rule] -> [(String, Int)]
buildMap rules = aux rules []
   where
   
   aux :: [Rule] -> [(String, Int)] -> [(String, Int)]
   aux [] m = m
   aux rules currentMap = let
      rulesWithVolume = map (\rule -> (rule, getVolume rule currentMap)) rules
      (solved, unsolved) = partition (\(_,mvol) -> isJust mvol) rulesWithVolume
      unpack (rule,mvol) = (container rule, fromJust mvol)
      in aux (map fst unsolved) (currentMap ++ map unpack solved)
   
   getVolume :: Rule -> [(String, Int)] -> Maybe Int
   getVolume rule currentMap = let
      subVolumes = map (\(q,d) -> fmap (*q) $ fmap (1+) $ lookup d currentMap) $ contents rule
      in sumMaybe subVolumes

sumMaybe :: [Maybe Int] -> Maybe Int
sumMaybe = foldr (liftA2 (+)) (Just 0)

part2 :: Input -> Int
part2 input = fromJust $ lookup "shiny gold" $ buildMap $ map parse input
