...
emptyPP :: Passport
emptyPP = Passport Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing

type Input = [String]

parse :: Input -> [Passport]
parse lines = do
   clump <- breakAll null lines
   let passportLine = intercalate " " clump
   let fields = breakAll (' '==) passportLine
   let fieldMap = map (tuplize . breakAll (==':')) fields
   -- let !x = debug True $ print fieldMap
   return $ buildPassportFrom emptyPP fieldMap
   where
   buildPassportFrom :: Passport -> [(String,String)] -> Passport
   buildPassportFrom p [] = p
   buildPassportFrom p ((k,v):xs) = case k of
      "byr" -> buildPassportFrom (p {byr = Just v}) xs
      "iyr" -> buildPassportFrom (p {iyr = Just v}) xs
      "eyr" -> buildPassportFrom (p {eyr = Just v}) xs
      "hgt" -> buildPassportFrom (p {hgt = Just v}) xs
      "hcl" -> buildPassportFrom (p {hcl = Just v}) xs
...
sequent :: Ord a => a -> a -> a -> Bool
sequent a b c = a <= b && b <= c
...
get :: (a -> Maybe b) -> a -> b
get f m = fromJust $ f m

valid :: Passport -> Bool
valid passport = let
   byrNum = read (get byr passport)
   iyrNum = read (get iyr passport)
   eyrNum = read (get eyr passport)
   (hgtNum,hgtUnit) = head $ reads (get hgt passport)
   (hclH:hclT) = get hcl passport
   in and
      [ length (get byr passport) == 4
      , all (`elem` digit) $ get byr passport
      , sequent 1920 byrNum 2002
      , length (get iyr passport) == 4
      , all (`elem` digit) $ get iyr passport
      , sequent 2010 iyrNum 2020
      , length (get eyr passport) == 4
      , all (`elem` digit) $ get eyr passport
      , sequent 2020 eyrNum 2030
      , case hgtUnit of { "cm" -> sequent 150 hgtNum 193; "in" -> sequent 59 hgtNum 76; _ -> False }
      , hclH == '#' && all (`elem` hex) hclT
      , (get ecl passport) `elem` ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
      , length (get pid passport) == 9
      , all (`elem` digit) $ get pid passport
      ]

part2 :: Input -> Int
part2 input = count valid $ filter (hasFields [byr,iyr,eyr,hgt,hcl,ecl,pid]) $ parse input
