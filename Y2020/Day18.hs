module AdventOfCode.Y2020.Day18 where
...
parse1 :: String -> Expr
parse1 = parseUsing parseExpr . filter (/=' ')
   where
   ...
   parseExpr = do
      term <- parseTerm
      oprs <- many parseOpr
      return $ foldl' (flip ($)) term oprs

   parseOpr :: Parser (Expr -> Expr)
   parseOpr = do
      c <- char
      e <- parseTerm
      case c of
   ...
   parseTerm = parseCon <|> parseSubExpr

   parseCon :: Parser Expr
   parseCon = fmap Con reader

part1 :: [String] -> Int
part1 input = sum $ map (eval . parse1) input



parse2 :: String -> Expr
parse2 = parseUsing (parseExpr 0) . filter (/=' ')
   where
   parseExpr :: Int -> Parser Expr
   parseExpr prec = parseTerm <|> case prec of
      0 -> parseMul
      1 -> parseAdd
      2 -> empty
   
   parseMul :: Parser Expr
   parseMul = parseAdd <|> do
      x <- parseExpr 1
      '*' <- char
      y <- parseExpr 0
      return $ Mul x y
...
part2 :: [String] -> Int
part2 input = sum $ map (eval . parse2) input
