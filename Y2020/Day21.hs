...
module AdventOfCode.Y2020.Day21 where
...
type Ingredient = String
   -- contains 0 or 1 allergen

type Allergen = String
   -- is contained in exactly 1 ingredient

-- from `a b c (contains x y)` we can deduce:
-- exactly one of {a,b,c} contains x
-- exactly one of {a,b,c} contains y
-- exactly one of {a,b,c} contains neither x or y
-- {a,b,c} might contain z

compileMap :: [Food] -> Map Allergen [Ingredient]
compileMap [] = M.empty
compileMap (f:fs) = foldr addAllergen (compileMap fs) (allergens f)
   where
   addAllergen :: Allergen -> Map Allergen [Ingredient] -> Map Allergen [Ingredient]
   addAllergen = M.alter $ \case
      { Nothing -> Just $ ingredients f
      ; Just ig -> Just $ intersect ig $ ingredients f }

getSafeIngredients :: [Food] -> [Ingredient]
getSafeIngredients foods = let
   m = compileMap foods
   maybeAllergenic = concat $ M.elems m
   allIngredients = nub (foods >>= ingredients)
   notAllergenic = allIngredients \\ maybeAllergenic
   in notAllergenic

part1 :: [Food] -> Int
part1 foods = let
   safeIngredients = getSafeIngredients foods
   allIngredients = foods >>= ingredients
   in count (`elem` safeIngredients) allIngredients

part2 :: [Food] -> String
part2 foods = let
   ingredientByAllergen = solveToBijection $ compileMap foods
   assocs = M.assocs ingredientByAllergen
   list = map snd $ sortOn fst assocs
   in intercalate "," list

parseFood :: String -> Food
parseFood str = let
   ingrListEnd = fromJust $ findIndex (=='(') str
   (a,b) = splitAt ingrListEnd str
   ingredients = init $ breakAll (==' ') a
   allergens = breakAll (==' ') $ filter (\c -> c /= ',' && c /= ')') $ drop 10 b
   in Food ingredients allergens

sample :: [Food]
sample = map parseFood
