module AdventOfCode.Y2023.Day03 where
...
part1 input = sum
   $ map (getValue input)
   $ filter (isPartNum input)
   $ getNumberPositions input

getNumberPositions :: Input -> [NumRef]
getNumberPositions grid = foldl' collect []
   $ sortOn swap
   $ findAll isNum grid
   where
   collect :: [NumRef] -> Coords -> [NumRef]
   collect [] c = [NumRef c 1]
   collect ((NumRef (x1,y1) n):ls) (x2,y2) = if y1 == y2 && x1 + n == x2
      then (NumRef (x1,y1) (n+1)) : ls
      else (NumRef (x2,y2) 1) : (NumRef (x1,y1) n) : ls

isPartNum :: Input -> NumRef -> Bool
isPartNum g (NumRef (x,y) n) = let
   neighbourCoords = [(x+dx,y+dy) | dx <- [-1..n], dy <- [-1..1]]
   in any (maybe False isSym . (g #)) neighbourCoords

getValue :: Input -> NumRef -> Int
getValue g = read . getValueStr
   where
   getValueStr (NumRef _ 0) = ""
   getValueStr (NumRef p n) = let
      curr = intToDigit $ fromNum (g #! p)
      next = getValueStr $ rightStep $ NumRef p n
      in curr:next
...
instance Read Cell where
   readsPrec _ (c:r)
      | isDigit c = [(Num (digitToInt c), r)]
      | c == '.' = [(Empty, r)]
      | otherwise = [(Sym c, r)]

parseInput :: [String] -> Input
parseInput sch = fromList $ map (map (\c -> read [c])) sch
