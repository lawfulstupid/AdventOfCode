...
data Cell = Ground | NorthSouth | EastWest | NorthWest | SouthWest | NorthEast | SouthEast | Animal
...
goes :: Direction -> Cell -> Bool
goes dir cell = elem dir $ directions cell
...
getCellFromDirections :: [Direction] -> Cell
getCellFromDirections dirs = let
   dirs' = sort dirs
   in head $ filter (\c -> directions c == dirs') [Ground ..]
...
part1 :: Input -> Int
part1 grid = loopLength grid `div` 2

getAnimalPipe :: Grid Cell -> Cell
getAnimalPipe grid = let
   p = getPos Animal grid
   go dir = grid # (step dir p) >>= \cell -> if goes (opposite dir) cell then Just dir else Nothing
   dirs = catMaybes [go North, go South, go East, go West]
...
parseInput lines = setGridLines False $ fromList $ flip map lines $ map $ \case
   '|' -> NorthSouth
   '-' -> EastWest
   'L' -> NorthEast
   'J' -> NorthWest
...
myInput :: Input
myInput = parseInput
   [ "FL7-|-FJ7.J.FF-.|7.JFF|F--7LJ-F--.--|.FF77LFL..7-F|-F|--L77F7-F7FFF--7---7FFL|7FJ7|J7.FF--F|7-F-7.L|FF|.FF7-J77F7FF7FF|.FF|7F7|-F.FJJFLJJFF-"
