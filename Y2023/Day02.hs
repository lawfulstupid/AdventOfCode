...
module AdventOfCode.Y2023.Day02 where
...
part1 :: Input -> Int
part1 = sum . map gameId . filter (all (<= targetHand) . hands )

targetHand = Hand 12 13 14

instance Ord Hand where
   a <= b = reds a <= reds b && greens a <= greens b && blues a <= blues b

part2 :: Input -> Integer
part2 = sum . map (power . handMax . hands)

handMax :: [Hand] -> Hand
handMax hands = Hand
   (maximum $ map reds hands)
   (maximum $ map greens hands)
   (maximum $ map blues hands)

power :: Hand -> Integer
power (Hand r g b) = fromIntegral (r*g*b)

-- Input parsing --

instance Parse [] Char Game where
   parser = do
      match "Game "
      n <- readParser
      match ": "
...
instance Parse [] Char Hand where
   parser = do
      h <- countParser
      t <- many (match ", " >> countParser)
      let list = (h:t)
      pure $ Hand
         (fromMaybe 0 $ lookup "red" list)
         (fromMaybe 0 $ lookup "green" list)
         (fromMaybe 0 $ lookup "blue" list)
      where
      countParser :: Parser [] Char (String, Int)
      countParser = do
         n <- readParser
         match " "
         col <- (match "red" <|> match "green" <|> match "blue")
         pure (col, n)

sampleInput :: Input
sampleInput = fullParse <$>
